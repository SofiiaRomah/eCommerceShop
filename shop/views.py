...
def add_to_cart_view(request, slug):
    ...
    if order:
        ...
        order_item = order.orderitem_set.filter(
            order=order, item=item_to_be_added
        ).first()
        if order_item:
            order_item.amount += 1
            order_item.save()
            ...
            order_item = OrderItem.objects.create(
                order=order, item=item_to_be_added, amount=1
            )
    ...
    order_item.save()
    messages.info(request, "This item was added to your shopping cart.")
...
def cart_view(request):
    ...
    for order_item in cart_items:
        cart_items_with_subtotal = {
            ...
            'subtotal': order_item.amount * order_item.item.price
        }
